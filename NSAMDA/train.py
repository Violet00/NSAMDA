...
from model import HardGAT, InnerProductDecoder, BilinearDecoder


def Train(directory, epochs, attn_size, num_heads, out_dim, feat_drop, attn_drop, slope, lr, wd, random_seed, cuda, model_type):
    ...
    g, disease_vertices, mirna_vertices, DiSM, MiSM, samples = build_graph(directory, random_seed)
    samples_df = pd.DataFrame(samples, columns=['drug', 'disease', 'label'])
    ...
    for train_idx, test_idx in kf.split(samples[:, 2]):
        ...
        if model_type == 'HGATMDA':
            model = HardGAT(G=g_train,
                            h_feats=attn_size,
                            num_heads=num_heads,
                            num_diseases=DiSM.shape[0],
                            num_mrnas=MiSM.shape[0],
                            d_sim_dim=DiSM.shape[1],
                            m_sim_dim=MiSM.shape[1],
                            out_dim=out_dim,
